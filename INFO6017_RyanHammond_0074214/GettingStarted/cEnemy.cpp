...
void cEnemy::SetPosition(glm::vec3& position)
{
...
void cEnemy::SetMeshEntry(int NumIndices, int BaseIndex, int BaseVertex)
{
	m_eMeshEntry.NumIndices = NumIndices;
	m_eMeshEntry.BaseIndex = BaseIndex;
	m_eMeshEntry.BaseVertex = BaseVertex;
}
void cEnemy::GetColor(glm::vec4& color)
{
	color = m_eColor;
}
void cEnemy::SetColor(glm::vec4& colorIn)
{
	m_eColor = colorIn;
}

void cEnemy::Scale(float scale)
{
...
void cEnemy::checkState(glm::mat4 targetTransform, glm::vec3 direction)
{
