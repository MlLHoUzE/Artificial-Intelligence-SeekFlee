...
int main()
{
	...
	{	//create player
		...
		gEntities.push_back(gPlayer);
	}
	success = LoadMesh("assets/bunny.obj", indices, vertices, monkeyMesh);
	//create 10 enemies
	for (int index = 0; index != 10; index++)
	{
		cEnemy* enemy = new cEnemy();
		enemy->SetPosition(glm::vec3(getRandFloat(gAreaInfo->Min.x, gAreaInfo->Max.x), 
									getRandFloat(gAreaInfo->Min.y, gAreaInfo->Max.y),
									getRandFloat(gAreaInfo->Min.z, gAreaInfo->Max.z)));
		enemy->Scale(7.0f);
		enemy->SetMeshEntry(monkeyMesh.NumIndices, monkeyMesh.BaseIndex, monkeyMesh.BaseVertex);
		gEntities.push_back(enemy);
	}
...
void DrawScene()
{
	...
	for (int index = 0; index != gEntities.size(); index++)
	{
		cEntity* curEntity = gEntities[index];
		glm::mat4 curTransform;
		curEntity->GetTransform(curTransform);
		glm::vec4 curColor;
		curEntity->GetColor(curColor);
		sMeshEntry curMeshEntry;
		curEntity->GetMeshEntry(curMeshEntry);
